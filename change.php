...
<html>
  ...
  <body>
    ...
    <?php
    $db = $itemName = $replacementValue = "";
    if ($_SERVER["REQUEST_METHOD"] == "POST") { 
      $db = fopen("data.txt", "r") or exit("The file is unable to be opened.");
      $itemName = $_POST["item-name-input"];
      $fieldToChange = $_POST["field-name-input"];
      $replacementValue = $_POST["change-field-input"];
      ...
      if ($fieldToChange == "Item Name") {

      } elseif($fieldToChange == "Inventory") {
        
      } elseif($fieldToChange == "Unit Type") {
        
      } elseif($fieldToChange == "Number Purchased") {
        
      } elseif($fieldToChange == "Number Sold") {
      ...
      function changeData($itemName, $fieldToChange, $replacementValue) {
